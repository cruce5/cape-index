...
// The §16 teaser shows a few hero portraits, self-hosted so the CSP img-src 'self' rule covers them. Only the ones the page uses ship.
mkdirSync(join(ROOT, "dist/reconcilers"), { recursive: true });
for (const f of readdirSync(join(ROOT, "assets/reconcilers"))) if (f.endsWith(".jpg") && doc.includes("reconcilers/" + f)) copyFileSync(join(ROOT, "assets/reconcilers", f), join(ROOT, "dist/reconcilers", f));
console.log(
